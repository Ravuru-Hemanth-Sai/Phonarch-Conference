...
type Bridge = { id: string; name: string; status: string; room_state?: string; host_name?: string; host_phone?: string; created_at?: string };
type Participant = {
...
type RoomState = { workspace_id?: string; bridge_id: string; room_state?: string; host?: { name: string; phone_number: string }; participants: Participant[]; speaker_requests?: SpeakerRequest[]; sessions?: SessionSummary[] };
...
function RoomPage({ room, state, tab, onTab, onRefresh, notify }: { room: Bridge; state: RoomState; tab: RoomTab; onTab: (tab: RoomTab) => void; onRefresh: () => void; notify: (message: string, tone?: "success" | "error") => void }) {
  ...
  return <div className="page-enter room-page"><div className="room-workspace"><section className="room-content"><PageHeader eyebrow={`Conference room · ${roomState}`} title={room.name} description="The fixed host is called first. Rostered listeners are called only after the host answers." actions={<><button type="button" className={`button ${running || starting ? "danger" : "primary"}`} onClick={() => void lifecycle(running || starting ? "stop" : "start")} disabled={starting}>{running || starting ? <><Ban size={15} /> Stop room</> : <><Phone size={15} /> Start room</>}</button><button type="button" className="button ghost" onClick={onRefresh}><RefreshCw size={15} /> Refresh</button></>} /><section className="room-summary"><div><span className="eyebrow">Room overview</span><h2>{running ? "Live operations" : starting ? "Calling host" : "Roster ready"}</h2><p className="subtle">{running ? "Active participants appear here while they are connected or dialing." : starting ? "The room will fan out to listeners after the host answers." : "Configure the host and fixed roster before starting this room."}</p></div><div className="room-summary-stats"><span><strong>{connected.length}</strong><small>Connected</small></span><span><strong>{live.length - connected.length}</strong><small>In progress</small></span><span><strong>{state.participants.filter((participant) => participant.role !== "HOST").length}</strong><small>Rostered</small></span></div></section>{tab === "live" && <LiveControl room={room} live={live} requests={state.speaker_requests || []} onAction={performAction} onVolume={readVolume} onSpeakerAction={speakerAction} onDial={dispatchFloatingDial} />}{tab === "dialer" && <DialerPage rooms={[room]} selectedRoomId={room.id} onRoomChange={() => undefined} onOpenRoom={() => { onTab("live"); onRefresh(); }} notify={notify} embedded />}{tab === "bulk" && <BulkPage rooms={[room]} selectedRoomId={room.id} onRoomChange={() => undefined} onOpenRoom={() => { onTab("live"); onRefresh(); }} notify={notify} embedded />}{tab === "activity" && <RoomActivity participants={state.participants} sessions={state.sessions || []} onAction={performAction} />}{tab === "settings" && <RoomSettingsPanel room={room} state={state} onRefresh={onRefresh} notify={notify} />}</section></div></div>;
}
...
function RoomSettingsPanel({ room, state, onRefresh, notify }: { room: Bridge; state: RoomState; onRefresh: () => void; notify: (message: string, tone?: "success" | "error") => void }) {
  ...
  const [hostPhone, setHostPhone] = useState(currentHost.phone_number);
  ...
  async function saveHost(event: FormEvent) {
    ...
    try { await request(`/api/v1/bridges/${room.id}/host`, { method: "PUT", body: JSON.stringify({ name: hostName.trim(), phone_number: hostPhone.trim() }) }); notify("Room host saved"); onRefresh(); } catch (error) { notify(error instanceof Error ? error.message : "Host could not be saved", "error"); } finally { setBusy(false); }
  }
...
function FloatingDialer({ onDial }: { onDial: (name: string, phone: string) => Promise<void> }) {
  ...
  async function submit(event: FormEvent) {
    ...
    setBusy(true);
    try { await onDial(name.trim() || phone.trim(), phone.trim()); setName(""); setPhone(""); setOpen(false); } catch { /* parent toast owns the error */ } finally { setBusy(false); }
  }
  return <div className="live-dialer"><button type="button" className="live-dialer-fab" title="Dial an ad-hoc participant" aria-label="Dial an ad-hoc participant" onClick={() => setOpen((value) => !value)}><Phone size={19} /></button>{open && <form className="live-dialer-popover panel" onSubmit={(event) => void submit(event)}><div className="eyebrow">Live room dialer</div><h3>Add a caller</h3><p className="subtle">This call is attached to the current room only.</p><input className="field-control" value={name} onChange={(event) => setName(event.target.value)} placeholder="Name" /><input className="field-control" value={phone} onChange={(event) => setPhone(event.target.value)} placeholder="Phone number" inputMode="tel" required /><button className="button primary" disabled={busy || !phone.trim()}><Phone size={14} /> {busy ? "Calling…" : "Dial participant"}</button></form>}</div>;
}
