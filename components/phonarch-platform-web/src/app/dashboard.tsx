...
function AdminRoomPolicy({ room, state, onRefresh, notify }: RoomSettingsProps) {
  ...
  return <section className="panel admin-room-policy-panel"><div className="settings-section-title"><div><div className="eyebrow">Product administration</div><h2>Room allocation</h2><p className="subtle">Only product administrators can assign the room TFN or change its concurrency limit.</p></div><ShieldCheck size={18} /></div><form className="admin-room-policy-form" onSubmit={(event) => void savePolicy(event)}><label className="field-label">Assigned TFN<select className="field-control" value={tfnID} onChange={(event) => setTFNID(event.target.value)} disabled={busy || locked}><option value="">No TFN assigned</option>{tfns.map((tfn) => <option key={tfn.id} value={tfn.id}>{tfn.number}{tfn.room_id && tfn.room_id !== room.id ? " · assigned" : ""}</option>)}</select></label><label className="field-label">Concurrency limit<input className="field-control" type="number" min="1" value={participantLimit} onChange={(event) => setParticipantLimit(event.target.value)} disabled={busy || locked} /></label><button className="button primary" disabled={busy || locked}>{locked ? "Locked while live" : <><Save size={14} /> Save allocation</>}</button></form></section>;
}
