...
function RoomSettingsPanel(props: RoomSettingsProps) {
  ...
  const [editingCard, setEditingCard] = useState<"host" | "participants" | null>(null);
  const host = props.state.host || { name: props.room.host_name || "", phone_number: props.room.host_phone || "" };
  const rosterCount = props.state.participants.filter((participant) => participant.role !== "HOST").length;
  return <div className="room-settings-stack"><section className="room-governance-card panel"><div><div className="eyebrow">Room launch requirements</div><h2>Room-scoped telephony</h2><p className="subtle">This room owns its caller identity and concurrency policy. Product allocation is intentionally isolated here.</p></div><div className="room-governance-items"><div><span>Participant limit</span><strong>{props.state.participant_limit || props.room.participant_limit || "—"}</strong><small>callers in this room</small></div><div className={!tfn ? "missing" : ""}><span>Assigned TFN</span><strong>{tfn || "Not assigned"}</strong><small>{tfn ? props.state.tfn?.label || props.room.tfn_label || "Room caller identity" : "Admin allocation required before start"}</small></div></div></section>{props.adminMode && <AdminRoomPolicy {...props} />}<section className="room-setting-cards"><article className="room-setting-card panel"><div className="room-setting-card-icon"><Users size={17} /></div><div className="room-setting-card-copy"><div className="eyebrow">Fixed host</div><h3>{host.name || "Host not configured"}</h3><p>{host.phone_number || "Add the host who will be called first when the room starts."}</p><span className="room-setting-card-meta"><Check size={12} /> {host.name && host.phone_number ? "Ready to call first" : "Configuration required"}</span></div><button type="button" className="button ghost compact-button" onClick={() => setEditingCard("host")}><Edit3 size={14} /> Edit card</button></article><article className="room-setting-card panel"><div className="room-setting-card-icon"><Users size={17} /></div><div className="room-setting-card-copy"><div className="eyebrow">Fixed participants</div><h3>{rosterCount} participant{rosterCount === 1 ? "" : "s"} in roster</h3><p>Listeners are called after the host answers and enter muted by default.</p><span className="room-setting-card-meta"><Check size={12} /> Room-only membership</span></div><button type="button" className="button ghost compact-button" onClick={() => setEditingCard("participants")}><Edit3 size={14} /> Edit card</button></article></section>{editingCard && <><div className="room-settings-edit-toolbar"><span><Edit3 size={14} /> Editing {editingCard === "host" ? "fixed host" : "fixed participants"}</span><button type="button" className="button ghost compact-button" onClick={() => setEditingCard(null)}>Done</button></div><RoomSettingsForm {...props} /></>}</div>;
}

function AdminRoomPolicy({ room, state, onRefresh, notify }: RoomSettingsProps) {
  ...
  return <section className="room-setting-card panel admin-room-policy-panel"><div className="room-setting-card-icon"><ShieldCheck size={17} /></div><div className="room-setting-card-copy"><div className="eyebrow">Product administration</div><h3>Room allocation</h3><p>Only product administrators can assign the room TFN or change its concurrency limit.</p><div className="room-policy-summary"><span><strong>{assignedTFN}</strong><small>Assigned TFN</small></span><span><strong>{participantLimit}</strong><small>Max callers</small></span></div></div><button type="button" className="button ghost compact-button" onClick={() => setEditing((value) => !value)} disabled={locked}><Edit3 size={14} /> {locked ? "Locked live" : editing ? "Close" : "Edit card"}</button>{editing && <form className="admin-room-policy-form" onSubmit={(event) => void savePolicy(event)}><label className="field-label">Assigned TFN<select className="field-control" value={tfnID} onChange={(event) => setTFNID(event.target.value)} disabled={busy || locked}><option value="">No TFN assigned</option>{tfns.map((tfn) => <option key={tfn.id} value={tfn.id}>{tfn.number}{tfn.room_id && tfn.room_id !== room.id ? " · assigned" : ""}</option>)}</select></label><label className="field-label">Concurrency limit<input className="field-control" type="number" min="1" value={participantLimit} onChange={(event) => setParticipantLimit(event.target.value)} disabled={busy || locked} /></label><button className="button primary" disabled={busy || locked}><Save size={14} /> Save allocation</button></form>}</section>;
}
