...
type DialRegion = { code: string; label: string; country: string; iso: string };
...
let requestScope: { admin: boolean; workspaceID: string } = { admin: false, workspaceID: "" };
...
const dialRegions: DialRegion[] = [
  { code: "+91", label: "India (+91)", country: "India", iso: "IN" },
  { code: "+1", label: "United States / Canada (+1)", country: "United States / Canada", iso: "US/CA" },
  { code: "+44", label: "United Kingdom (+44)", country: "United Kingdom", iso: "GB" },
  { code: "+61", label: "Australia (+61)", country: "Australia", iso: "AU" },
  { code: "+65", label: "Singapore (+65)", country: "Singapore", iso: "SG" },
  { code: "+971", label: "United Arab Emirates (+971)", country: "United Arab Emirates", iso: "AE" },
];

function regionForPhone(phone: string, fallback: string): string {
  const digits = phone.replace(/\D/g, "");
  return [...dialRegions].sort((left, right) => right.code.length - left.code.length).find((region) => digits.startsWith(region.code.replace(/\D/g, "")))?.code || fallback;
}

function localDigitsForRegion(phone: string, region: string): string {
  const digits = phone.replace(/\D/g, "");
  const regionDigits = region.replace(/\D/g, "");
  return digits.startsWith(regionDigits) ? digits.slice(regionDigits.length) : digits;
}
...
async function request<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  const response = await fetch(`${API}${scopedPath}`, { ...init, headers, credentials: "include", cache: "no-store" });
  const raw = await response.text();
  let payload: unknown = {};
  try { payload = raw ? JSON.parse(raw) : {}; } catch { payload = {}; }
  if (response.status === 401) {
    window.location.href = requestScope.admin ? "/admin/login" : "/login";
...
function RegionSelector({ value, onChange, disabled = false, ariaLabel = "Phone region", compact = false }: { value: string; onChange: (value: string) => void; disabled?: boolean; ariaLabel?: string; compact?: boolean }) {
  ...
  return <div className={`region-selector ${compact ? "compact" : ""}`} ref={selectorRef}><button type="button" className={`region-selector-trigger ${open ? "open" : ""}`} aria-label={`${ariaLabel}: ${selected.country} ${selected.code}`} title={selected.country} aria-haspopup="listbox" aria-expanded={open} onClick={() => setOpen((current) => !current)} disabled={disabled}>{compact ? <><span className="region-selector-iso">{selected.iso}</span><span className="region-selector-compact-code">{selected.code}</span></> : <><span className="region-selector-country">{selected.country}</span><span className="region-selector-dial-code">{selected.code}</span></>}<ChevronDown size={15} className="region-selector-chevron" /></button>{open && <div className="region-selector-popover" role="listbox" aria-label={ariaLabel}>{dialRegions.map((region) => <button type="button" role="option" aria-selected={region.code === selected.code} className={`region-selector-option ${region.code === selected.code ? "active" : ""}`} key={region.code} onClick={() => { onChange(region.code); setOpen(false); }}><span className="region-selector-option-copy"><strong>{region.country}</strong><small>{region.iso} · {region.code}</small></span>{region.code === selected.code && <Check size={15} />}</button>)}</div>}</div>;
}
...
export function Dashboard({ adminMode = false }: { adminMode?: boolean }) {
  ...
  const selectedWorkspace = adminWorkspaces.find((workspace) => workspace.id === selectedWorkspaceID);
  ...
  const loadRooms = useCallback(async () => {
    if (adminMode && !selectedWorkspaceID) return;
    try {
  ...
  }, [adminMode, notify, selectedWorkspaceID]);

  const loadIdentity = useCallback(async () => {
    try {
      if (adminMode) {
        const admin = await request<{ username: string; display_name: string; platform_admin: boolean }>("/api/v1/auth/admin/me");
        setIdentity({ username: admin.username, display_name: admin.display_name, platform_admin: admin.platform_admin, workspace_role: "PRODUCT_ADMIN", can_manage_users: true });
      } else {
        setIdentity(await request<AuthMe>("/api/v1/auth/me"));
      }
    } catch (error) { notify(error instanceof Error ? error.message : "Session could not be loaded", "error"); }
  }, [adminMode, notify]);

  const loadAdminWorkspaces = useCallback(async () => {
    if (!adminMode) return;
    try {
      const data = await request<WorkspaceOption[]>("/api/v1/admin/workspaces");
      setAdminWorkspaces(data || []);
      setSelectedWorkspaceID((current) => current && data.some((workspace) => workspace.id === current) ? current : data[0]?.id || "");
    } catch (error) { notify(error instanceof Error ? error.message : "Workspaces could not be loaded", "error"); }
  }, [adminMode, notify]);
  ...
  useEffect(() => {
    requestScope = { admin: adminMode, workspaceID: adminMode ? selectedWorkspaceID : "" };
  }, [adminMode, selectedWorkspaceID]);
  useEffect(() => { void loadIdentity(); if (adminMode) void loadAdminWorkspaces(); }, [adminMode, loadAdminWorkspaces, loadIdentity]);
  useEffect(() => { if (!adminMode || selectedWorkspaceID) void loadRooms(); }, [adminMode, loadRooms, selectedWorkspaceID]);
  useEffect(() => {
    if (!rooms.length) return;
    void loadAllRooms();
...
function RoomPage({ room, state, tab, onTab, onRefresh, notify, adminMode = false }: { room: Bridge; state: RoomState; tab: RoomTab; onTab: (tab: RoomTab) => void; onRefresh: () => void; notify: (message: string, tone?: "success" | "error") => void; adminMode?: boolean }) {
...
function RoomTabButton({ active, onClick, icon: Icon, label, detail }: { active: boolean; onClick: () => void; icon: LucideIcon; label: string; detail: string }) {
  return <button type="button" role="tab" aria-selected={active} aria-label={`${label}: ${detail}`} title={label} className={`room-tab ${active ? "active" : ""}`} onClick={onClick}><span className="room-tab-icon"><Icon size={16} /></span><span><strong>{label}</strong><small>{detail}</small></span><ChevronRight size={15} className="room-tab-chevron" /></button>;
}
...
type RoomSettingsProps = { room: Bridge; state: RoomState; onRefresh: () => void; notify: (message: string, tone?: "success" | "error") => void; adminMode?: boolean };

function RoomSettingsPanel(props: RoomSettingsProps) {
  return <div className="room-settings-stack"><div className="room-settings-intro-card"><div><div className="eyebrow">Room configuration</div><h2>Set the room once, then operate with confidence.</h2><p>These controls apply only to <strong>{props.room.name}</strong>. Configure the dialing region first, then the room caller identity, host, capacity, and fixed roster.</p></div><span className="room-settings-scope"><ShieldCheck size={15} /> Room-scoped</span></div><RoomSettingsFormNew {...props} /></div>;
}

function AdminRoomPolicyLegacy({ room, state, onRefresh, notify, adminMode = false }: RoomSettingsProps) {
  const [tfns, setTFNs] = useState<Array<{ id: string; number: string; label: string; room_id?: string }>>([]);
  const defaultPolicyRegion = state.default_region || room.default_region || "+91";
  const currentTFN = state.tfn?.number || room.tfn_number || "";
  const initialTFNRegion = regionForPhone(currentTFN, defaultPolicyRegion);
  const [tfnRegion, setTFNRegion] = useState(initialTFNRegion);
  const [tfnLocalNumber, setTFNLocalNumber] = useState(localDigitsForRegion(currentTFN, initialTFNRegion));
  ...
  useEffect(() => {
    const nextTFN = state.tfn?.number || room.tfn_number || "";
    const nextRegion = regionForPhone(nextTFN, state.default_region || room.default_region || "+91");
    setParticipantLimit(String(state.participant_limit || room.participant_limit || 500));
    setTFNRegion(nextRegion);
    setTFNLocalNumber(localDigitsForRegion(nextTFN, nextRegion));
  }, [room.default_region, room.participant_limit, room.tfn_number, state.default_region, state.participant_limit, state.tfn?.number]);
  ...
  async function savePolicy(event: FormEvent) {
    ...
    try {
      const normalizedTFN = tfnLocalNumber.trim() ? normalizeE164(`${tfnRegion}${tfnLocalNumber}`) : "";
      ...
      setTFNRegion(regionForPhone(normalizedTFN, tfnRegion));
      setTFNLocalNumber(localDigitsForRegion(normalizedTFN, tfnRegion));
      setEditing(false); notify("Room TFN / DID and participant limit saved"); onRefresh();
    } catch (error) { notify(error instanceof Error ? error.message : "Room policy could not be saved", "error"); } finally { setBusy(false); }
  }

  const assignedTFN = tfnLocalNumber ? normalizeE164(`${tfnRegion}${tfnLocalNumber}`) : "Not assigned";
  const datalistID = `room-tfns-${room.id}`;
  return <section className="room-setting-card panel admin-room-policy-panel"><div className="room-setting-card-icon"><ShieldCheck size={17} /></div><div className="room-setting-card-copy"><div className="eyebrow">02 · Room telephony</div><h3>{adminMode ? "TFN / DID and capacity" : "Product allocation"}</h3><p>{adminMode ? "Choose the room caller identity and concurrency limit. The number is stored in E.164 format." : "The product administrator controls this room’s caller identity and concurrency limit."}</p><div className="room-policy-summary"><span><strong>{assignedTFN}</strong><small>Assigned TFN / DID</small></span><span><strong>{participantLimit}</strong><small>Max callers</small></span></div></div>{adminMode ? <button type="button" className="button ghost compact-button" onClick={() => setEditing((value) => !value)} disabled={locked}><Edit3 size={14} /> {locked ? "Locked live" : editing ? "Close" : "Edit card"}</button> : <span className="ownership-label"><LockKeyhole size={12} /> View only</span>}{editing && <form className="admin-room-policy-form" onSubmit={(event) => void savePolicy(event)}><label className="field-label">Room TFN / DID<span className="phone-composite tfn-phone-composite"><RegionSelector value={tfnRegion} onChange={setTFNRegion} disabled={busy || locked} ariaLabel="TFN country and E.164 region" /><input className="field-control" type="tel" inputMode="numeric" list={datalistID} value={tfnLocalNumber} onChange={(event) => setTFNLocalNumber(event.target.value.replace(/\D/g, "").slice(0, 15))} placeholder="9876543210" aria-label="TFN / DID local number" maxLength={15} disabled={busy || locked} /></span><datalist id={datalistID}>{tfns.filter((tfn) => regionForPhone(tfn.number, tfnRegion) === tfnRegion).map((tfn) => <option key={tfn.id} value={localDigitsForRegion(tfn.number, tfnRegion)}>{tfn.label || tfn.number}</option>)}</datalist><small className="field-hint">Select a country, then enter the local digits. The saved value will be E.164, for example {tfnRegion} 9876543210.</small></label><label className="field-label">Concurrency limit<input className="field-control" type="number" min="1" value={participantLimit} onChange={(event) => setParticipantLimit(event.target.value)} disabled={busy || locked} /></label><button className="button primary" disabled={busy || locked}><Save size={14} /> Save TFN / DID</button></form>}</section>;
}
...
function AdminRoomPolicy({ room, state, onRefresh, notify, adminMode = false }: RoomSettingsProps) {
  ...
  async function savePolicy(event: FormEvent) {
    ...
    try {
      const normalizedTFN = tfnLocalNumber.trim() ? normalizeE164(`${tfnRegion}${tfnLocalNumber}`) : "";
      if (normalizedTFN && !/^\+\d{8,15}$/.test(normalizedTFN)) throw new Error("TFN / DID must be a valid phone number with a country code");
      let tfnID = "";
      if (normalizedTFN) {
        const existing = tfns.find((tfn) => normalizeE164(tfn.number) === normalizedTFN);
        if (existing) {
  ...
  const assignedTFN = tfnLocalNumber ? normalizeE164(`${tfnRegion}${tfnLocalNumber}`) : "Not assigned";
  const datalistID = `room-tfns-${room.id}-customer`;
  return <section className="room-setting-card panel admin-room-policy-panel"><div className="room-setting-card-icon"><ShieldCheck size={17} /></div><div className="room-setting-card-copy"><div className="eyebrow">02 · Room telephony</div><h3>{adminMode ? "TFN / DID and capacity" : "Product allocation"}</h3><p>{adminMode ? "Choose the room caller identity and caller limit. The country code is saved with the number." : "The product administrator controls this room’s caller identity and concurrency limit."}</p><div className="room-policy-summary"><span><strong>{assignedTFN}</strong><small>Assigned TFN / DID</small></span><span><strong>{participantLimit}</strong><small>Max callers</small></span></div></div>{adminMode ? <button type="button" className="button ghost compact-button" onClick={() => setEditing((value) => !value)} disabled={locked}><Edit3 size={14} /> {locked ? "Locked live" : editing ? "Close" : "Edit card"}</button> : <span className="ownership-label"><LockKeyhole size={12} /> View only</span>}{editing && <form className="admin-room-policy-form" onSubmit={(event) => void savePolicy(event)}><label className="field-label">Room TFN / DID<span className="phone-composite tfn-phone-composite"><RegionSelector value={tfnRegion} onChange={setTFNRegion} compact disabled={busy || locked} ariaLabel="TFN country code" /><input className="field-control" type="tel" inputMode="numeric" list={datalistID} value={tfnLocalNumber} onChange={(event) => setTFNLocalNumber(event.target.value.replace(/\D/g, "").slice(0, 15))} placeholder="9876543210" aria-label="TFN / DID local number" maxLength={15} disabled={busy || locked} /></span><datalist id={datalistID}>{tfns.filter((tfn) => regionForPhone(tfn.number, tfnRegion) === tfnRegion).map((tfn) => <option key={tfn.id} value={localDigitsForRegion(tfn.number, tfnRegion)}>{tfn.label || tfn.number}</option>)}</datalist><small className="field-hint">Choose the country code, then enter the local number. Example: {tfnRegion} 9876543210.</small></label><label className="field-label">Concurrency limit<input className="field-control" type="number" min="1" value={participantLimit} onChange={(event) => setParticipantLimit(event.target.value)} disabled={busy || locked} /></label><button className="button primary" disabled={busy || locked}><Save size={14} /> Save TFN / DID</button></form>}</section>;
}

function RoomSettingsForm({ room, state, onRefresh, notify, adminMode = false }: RoomSettingsProps) {
  const currentHost = state.host || { name: room.host_name || "", phone_number: room.host_phone || "" };
  const initialDefaultRegion = state.default_region || room.default_region || "+91";
  const initialHostRegion = regionForPhone(currentHost.phone_number, initialDefaultRegion);
  const [defaultRegion, setDefaultRegion] = useState(initialDefaultRegion);
  const [hostName, setHostName] = useState(currentHost.name);
  const [hostRegion, setHostRegion] = useState(initialHostRegion);
  const [hostPhone, setHostPhone] = useState(localDigitsForRegion(currentHost.phone_number, initialHostRegion));
  const [participantName, setParticipantName] = useState("");
  const [participantRegion, setParticipantRegion] = useState(initialDefaultRegion);
  const [participantPhone, setParticipantPhone] = useState("");
  const [editingParticipant, setEditingParticipant] = useState<Participant | null>(null);
  const [editingName, setEditingName] = useState("");
  const [editingRegion, setEditingRegion] = useState(initialDefaultRegion);
  ...
  async function addParticipant(event: FormEvent) {
    ...
    setBusy(true);
    try { await request(`/api/v1/bridges/${room.id}/participants`, { method: "POST", body: JSON.stringify({ name: participantName.trim(), phone_number: normalizePhone(participantPhone, participantRegion), role: "LISTENER" }) }); setParticipantName(""); setParticipantPhone(""); notify("Participant added to the fixed roster"); onRefresh(); } catch (error) { notify(error instanceof Error ? error.message : "Participant could not be added", "error"); } finally { setBusy(false); }
  }
  ...
  function beginEdit(participant: Participant) { const region = regionForPhone(participant.phone_number, defaultRegion); setEditingParticipant(participant); setEditingName(participant.name); setEditingRegion(region); setEditingPhone(localDigitsForRegion(participant.phone_number, region)); }
...
function RoomSettingsFormNew({ room, state, onRefresh, notify, adminMode = false }: RoomSettingsProps) {
  ...
  async function saveHost(event: FormEvent) {
    event.preventDefault();
    if (!hasTenLocalDigits(hostPhone)) { notify("Host phone number must be exactly 10 digits after the country code", "error"); return; }
    setBusy(true);
    try { await request(`/api/v1/bridges/${room.id}/host`, { method: "PUT", body: JSON.stringify({ name: hostName.trim(), phone_number: normalizePhone(hostPhone, hostRegion) }) }); await request(`/api/v1/bridges/${room.id}/settings`, { method: "PUT", body: JSON.stringify({ default_region: defaultRegion }) }); notify("Room host and dialing defaults saved"); onRefresh(); } catch (error) { notify(error instanceof Error ? error.message : "Host could not be saved", "error"); } finally { setBusy(false); }
  }
  async function clearHost() { setBusy(true); try { await request(`/api/v1/bridges/${room.id}/host`, { method: "DELETE" }); setHostName(""); setHostPhone(""); notify("Room host cleared"); onRefresh(); } catch (error) { notify(error instanceof Error ? error.message : "Host could not be cleared", "error"); } finally { setBusy(false); } }
  async function addParticipant(event: FormEvent) {
    event.preventDefault();
    if (!hasTenLocalDigits(participantPhone)) { notify("Participant phone number must be exactly 10 digits after the country code", "error"); return; }
    setBusy(true);
    try { await request(`/api/v1/bridges/${room.id}/participants`, { method: "POST", body: JSON.stringify({ name: participantName.trim(), phone_number: normalizePhone(participantPhone, participantRegion), role: "LISTENER" }) }); setParticipantName(""); setParticipantPhone(""); notify("Participant added to the fixed roster"); onRefresh(); } catch (error) { notify(error instanceof Error ? error.message : "Participant could not be added", "error"); } finally { setBusy(false); }
  }
  async function removeParticipant(id: string) { try { await request(`/api/v1/participants/${id}/remove`, { method: "POST", body: "{}" }); notify("Participant removed from the fixed roster"); onRefresh(); } catch (error) { notify(error instanceof Error ? error.message : "Participant could not be removed", "error"); } }
  ...
  async function saveParticipant(event: FormEvent) {
    event.preventDefault();
    if (!editingParticipant) return;
    if (!hasTenLocalDigits(editingPhone)) { notify("Participant phone number must be exactly 10 digits after the country code", "error"); return; }
    setBusy(true);
    try { await request(`/api/v1/participants/${editingParticipant.participant_id}`, { method: "PUT", body: JSON.stringify({ name: editingName.trim(), phone_number: normalizePhone(editingPhone, editingRegion) }) }); setEditingParticipant(null); notify("Participant roster details saved"); onRefresh(); } catch (error) { notify(error instanceof Error ? error.message : "Participant could not be saved", "error"); } finally { setBusy(false); }
  }
  const roster = state.participants.filter((participant) => participant.role !== "HOST");
  return <section className="room-settings-grid"><section className="panel settings-main room-region-card"><div className="settings-section-title"><div><div className="eyebrow">01 · Room dialing region</div><h2>Default phone region</h2><p className="subtle">Choose the country once. Host, roster, and live dialer inputs use this region by default.</p></div><Network size={22} /></div><form className="room-region-form" onSubmit={(event) => void saveDialingRegion(event)}><label className="field-label">Default region<RegionSelector value={defaultRegion} onChange={setDefaultRegion} ariaLabel="Default phone region" /></label><div className="room-region-example"><span>Local number format</span><strong>{defaultRegion} + 10 digits</strong><small>Example: {defaultRegion} 9876543210</small></div><button className="button primary compact-button" disabled={busy}><Save size={14} /> Save region</button></form></section><AdminRoomPolicy room={room} state={state} onRefresh={onRefresh} notify={notify} adminMode={adminMode} /><div className="panel settings-main host-panel"><div className="settings-section-title"><div><div className="eyebrow">03 · Room host</div><h2>Fixed host details</h2><p className="subtle">The host is always called first when this room starts. Edit these details while the room is stopped.</p></div><ShieldCheck size={22} /></div><form className="host-form" onSubmit={(event) => void saveHost(event)}><label className="field-label">Host name<input className="field-control" value={hostName} onChange={(event) => setHostName(event.target.value)} placeholder="e.g. Maya Chen" required /></label><label className="field-label">Host phone number<span className="phone-composite"><RegionSelector value={hostRegion} onChange={setHostRegion} compact ariaLabel="Host phone country code" /><input className="field-control" value={hostPhone} onChange={(event) => setHostPhone(localPhoneDigits(event.target.value))} placeholder="9876543210" inputMode="numeric" minLength={10} maxLength={10} pattern="[0-9]{10}" aria-label="Host phone local number" required /></span><small className="phone-hint">Choose a country code and enter exactly 10 local digits.</small>{normalizePhone(hostPhone, hostRegion) && <small className="phone-preview">Will dial {normalizePhone(hostPhone, hostRegion)}</small>}</label><div className="settings-action-row"><button className="button primary" disabled={busy}>{busy ? "Saving…" : "Save host"}</button><button type="button" className="button ghost danger-button" onClick={() => void clearHost()} disabled={busy || !hostName}>Clear host</button></div></form></div><div className="panel settings-main roster-panel"><div className="settings-section-title"><div><div className="eyebrow">04 · Fixed participants</div><h2>Roster before the call</h2><p className="subtle">Listeners join muted by default. They stay assigned to this room across sessions.</p></div><Users size={22} /></div><form className="roster-add-form" onSubmit={(event) => void addParticipant(event)}><input className="field-control" value={participantName} onChange={(event) => setParticipantName(event.target.value)} placeholder="Participant name" aria-label="Participant name" required /><span className="phone-composite"><RegionSelector value={participantRegion} onChange={setParticipantRegion} compact ariaLabel="Participant phone country code" /><input className="field-control" value={participantPhone} onChange={(event) => setParticipantPhone(localPhoneDigits(event.target.value))} placeholder="9876543210" aria-label="Participant phone local number" inputMode="numeric" minLength={10} maxLength={10} pattern="[0-9]{10}" required /></span><button className="button primary" disabled={busy}><Plus size={14} /> Add participant</button></form><small className="phone-hint roster-phone-hint">Choose a country code and enter exactly 10 local digits.</small>{roster.length ? <div className="roster-list">{roster.map((participant) => editingParticipant?.participant_id === participant.participant_id ? <form className="roster-edit-row" key={participant.participant_id} onSubmit={(event) => void saveParticipant(event)}><input className="field-control" value={editingName} onChange={(event) => setEditingName(event.target.value)} aria-label="Edit participant name" required /><span className="phone-composite"><RegionSelector value={editingRegion} onChange={setEditingRegion} compact ariaLabel="Edit participant phone country code" /><input className="field-control" value={editingPhone} onChange={(event) => setEditingPhone(localPhoneDigits(event.target.value))} aria-label="Edit participant phone number" inputMode="numeric" minLength={10} maxLength={10} pattern="[0-9]{10}" required /></span><button className="button primary compact-button" disabled={busy}>Save</button><button type="button" className="button ghost compact-button" onClick={() => setEditingParticipant(null)}>Cancel</button></form> : <div className="roster-row" key={participant.participant_id}><span className="participant-avatar">{initials(participant.name || participant.phone_number)}</span><span><strong>{participant.name || "Unnamed participant"}</strong><small>{participant.phone_number}</small></span><span className="role-pill">Listener</span><span className="roster-row-actions"><button type="button" className="icon-button" title="Edit participant" onClick={() => beginEdit(participant)}><Pencil size={14} /></button><button type="button" className="icon-button danger" title="Remove participant from roster" onClick={() => void removeParticipant(participant.participant_id)}><Trash2 size={14} /></button></span></div>)}</div> : <div className="session-empty"><Users size={17} /><span><strong>No fixed participants</strong><small>Add them manually or import a Name, Phone Number, Role file.</small></span></div>}</div></section>;
}
...
type WorkspaceSettingsPane = "profile" | "users" | "roles";

const workspaceSettingsItems: Array<{ id: WorkspaceSettingsPane; label: string; description: string; icon: LucideIcon }> = [
  { id: "profile", label: "Workspace profile", description: "Identity and boundary", icon: Building2 },
  { id: "users", label: "User access", description: "Members and invitations", icon: Users },
  { id: "roles", label: "Roles & permissions", description: "Workspace access policy", icon: LockKeyhole },
];

const workspaceRoleDescriptions: Record<string, { label: string; description: string; permissions: string[] }> = {
  OWNER: { label: "Owner", description: "Full control and ownership", permissions: ["Manage members", "Manage room operations", "Manage workspace profile", "Inspect activity"] },
  ADMIN: { label: "Admin", description: "Manage configuration and members", permissions: ["Manage members", "Manage room operations", "Inspect activity"] },
  OPERATOR: { label: "Operator", description: "Operate and inspect live rooms", permissions: ["Start and stop rooms", "Mute and unmute callers", "Inspect activity"] },
  VIEWER: { label: "Viewer", description: "Read-only workspace access", permissions: ["Inspect room status", "Inspect activity"] },
};

function WorkspaceSettingsFrame({ identity, adminMode, workspaceName, pane, onPaneChange, children }: { identity: AuthMe | null; adminMode: boolean; workspaceName: string; pane: WorkspaceSettingsPane; onPaneChange: (pane: WorkspaceSettingsPane) => void; children: React.ReactNode }) {
  const active = workspaceSettingsItems.find((item) => item.id === pane) || workspaceSettingsItems[0];
  const ActiveIcon = active.icon;
  return <div className="page-enter workspace-settings-page"><section className="workspace-settings-intro"><div><div className="eyebrow">Workspace settings</div><h1>Connections and access, in one boundary.</h1><p className="subtle">Keep identity, team permissions, and room operations inside the selected workspace.</p></div><div className="settings-boundary"><ShieldCheck size={15} /><span><strong>{workspaceName || "Workspace"}</strong><small>{adminMode ? "Product-admin scoped workspace" : `${identity?.workspace_role || "Workspace"} scoped access`}</small></span></div></section><div className="workspace-settings-shell"><nav className="workspace-settings-nav" aria-label="Workspace settings">{workspaceSettingsItems.map((item) => { const ItemIcon = item.icon; return <button type="button" key={item.id} className={`workspace-settings-nav-item ${pane === item.id ? "active" : ""}`} onClick={() => onPaneChange(item.id)}><ItemIcon size={16} /><span><strong>{item.label}</strong><small>{item.description}</small></span><ChevronRight size={14} /></button>; })}</nav><section className="workspace-settings-content"><div className="workspace-settings-content-head"><div className="workspace-settings-content-title"><div className="settings-title-icon"><ActiveIcon size={18} /></div><div><div className="eyebrow">Workspace configuration</div><h2>{active.label}</h2><p className="subtle">{active.description} for {workspaceName || "the selected workspace"}.</p></div></div><span className="settings-scope"><span /> {adminMode ? "Product admin scope" : "Workspace scoped"}</span></div>{children}</section></div></div>;
}

function WorkspaceProfilePanel({ identity, workspaceName, adminMode }: { identity: AuthMe | null; workspaceName: string; adminMode: boolean }) {
  return <div className="settings-form-stack"><div className="settings-callout"><div className="callout-icon"><Building2 size={16} /></div><div><strong>{workspaceName || "Workspace"} is the customer boundary</strong><p>Users, rooms, call history, and room-level telephony configuration remain isolated here.</p></div><span className="status live">Protected</span></div><div className="settings-form-card workspace-profile-card"><div className="subsection-head"><div><h3>Workspace identity</h3><p className="subtle">{adminMode ? "Product administrators can manage this workspace profile." : "This workspace profile is managed by the product administrator."}</p></div><ShieldCheck size={17} /></div><div className="workspace-profile-grid"><div><span>Workspace</span><strong>{workspaceName || "—"}</strong></div><div><span>Access role</span><strong>{adminMode ? "Product admin" : identity?.workspace_role || "Workspace user"}</strong></div><div><span>Room boundary</span><strong>Workspace isolated</strong></div><div><span>Media policy</span><strong>SIP + RTP only</strong></div></div></div><div className="settings-note"><ShieldCheck size={15} /><span><strong>Room-specific policy.</strong> TFN/DID and concurrency are intentionally managed inside each room’s Settings, never from this workspace surface.</span></div></div>;
}

function WorkspaceRolesPanel() {
  return <div className="settings-form-stack"><div className="settings-callout"><div className="callout-icon"><LockKeyhole size={16} /></div><div><strong>Simple, explicit workspace RBAC</strong><p>Roles control workspace actions without exposing platform administration.</p></div><span className="status live">4 roles</span></div><div className="workspace-role-matrix">{Object.entries(workspaceRoleDescriptions).map(([role, value]) => <article className="workspace-role-card" key={role}><div><span className="role-chip">{role}</span><h3>{value.label}</h3><p>{value.description}</p></div><div className="permission-list">{value.permissions.map((permission) => <span key={permission}><Check size={12} /> {permission}</span>)}</div></article>)}</div><div className="settings-note"><LockKeyhole size={15} /><span><strong>Ownership guard.</strong> The workspace owner cannot be removed or demoted by workspace users.</span></div></div>;
}

function SettingsPage({ identity, adminMode = false, workspaces = [], selectedWorkspaceID = "", onWorkspaceCreated }: { identity: AuthMe | null; adminMode?: boolean; workspaces?: WorkspaceOption[]; selectedWorkspaceID?: string; onWorkspaceCreated?: () => void }) {
  ...
  const [pane, setPane] = useState<WorkspaceSettingsPane>("users");
  ...
  if (adminMode) {
    return <ProductAdminSettings identity={identity} workspaces={workspaces} selectedWorkspaceID={selectedWorkspaceID} onWorkspaceCreated={onWorkspaceCreated} />;
  }
  ...
  return <WorkspaceSettingsFrame identity={identity} adminMode={false} workspaceName={identity?.workspace?.name || "Workspace"} pane={pane} onPaneChange={setPane}>{pane === "profile" && <WorkspaceProfilePanel identity={identity} workspaceName={identity?.workspace?.name || "Workspace"} adminMode={false} />}{pane === "roles" && <WorkspaceRolesPanel />}{pane === "users" && <div className="settings-form-stack"><div className="settings-callout"><div className="callout-icon"><Users size={16} /></div><div><strong>Access is isolated to this workspace</strong><p>Members can have different responsibilities here without changing their identity elsewhere.</p></div><span className="status live">{members.length} member{members.length === 1 ? "" : "s"}</span></div>{canManageUsers ? <div className="settings-form-card"><div className="subsection-head"><div><h3>Invite a local workspace member</h3><p className="subtle">Create a local account with a temporary password. Share it securely and ask the member to change it after sign-in.</p></div><UserPlus size={17} /></div><form className="form-grid workspace-member-form" onSubmit={(event) => void addUser(event)}><label className="field-label">Display name<input className="field-control" value={displayName} onChange={(event) => setDisplayName(event.target.value)} placeholder="Alex Morgan" required /></label><label className="field-label">Username<input className="field-control" value={username} onChange={(event) => setUsername(event.target.value)} placeholder="alex.morgan" required /></label><label className="field-label">Role<select className="field-control" value={role} onChange={(event) => setRole(event.target.value)}><option value="OPERATOR">Operator</option><option value="ADMIN">Workspace admin</option><option value="VIEWER">Viewer</option></select></label><label className="field-label">Temporary password<input className="field-control" type="password" minLength={8} value={password} onChange={(event) => setPassword(event.target.value)} placeholder="At least 8 characters" required /></label><div className="editor-actions"><button className="button primary" disabled={busy}>{busy ? "Adding…" : <><Plus size={14} /> Add member</>}</button></div></form></div> : <div className="settings-form-card settings-readonly-card"><LockKeyhole size={16} /><span><strong>Read-only access management</strong><small>Only workspace owners and admins can add, edit, or remove members.</small></span></div>}{message && <div className="settings-inline-message">{message}</div>}<div className="member-list">{members.map((member) => <div className="member-card" key={member.operator_id}><div className="member-avatar">{initials(member.display_name || member.username)}</div><div className="member-copy"><strong>{member.display_name || member.username}</strong><small>{member.username}</small></div><span className="role-chip">{workspaceRoleDescriptions[member.role]?.label || member.role}</span><span className={`member-status ${member.status === "ACTIVE" ? "active" : "paused"}`}>{member.status === "ACTIVE" ? "Active" : member.status}</span><div className="member-card-actions">{member.role === "OWNER" ? <span className="ownership-label">Owner</span> : canManageUsers ? <><select className="field-control compact-select" value={member.role} onChange={(event) => void updateRole(member, event.target.value)} disabled={busy} aria-label={`Role for ${member.display_name || member.username}`}><option value="ADMIN">Admin</option><option value="OPERATOR">Operator</option><option value="VIEWER">Viewer</option></select><button type="button" className="icon-button danger" onClick={() => void removeUser(member)} disabled={busy} title={`Remove ${member.display_name || member.username}`}><Trash2 size={14} /></button></> : <span className="ownership-label">View only</span>}</div></div>)}{!members.length && <div className="empty compact-empty">No workspace users yet.</div>}</div></div>}</WorkspaceSettingsFrame>;
...
function ProductAdminSettings({ identity, workspaces, selectedWorkspaceID, onWorkspaceCreated }: { identity: AuthMe | null; workspaces: WorkspaceOption[]; selectedWorkspaceID: string; onWorkspaceCreated?: () => void }) {
  ...
  const [role, setRole] = useState("OPERATOR");
  const [members, setMembers] = useState<WorkspaceMember[]>([]);
  const [lastAccess, setLastAccess] = useState<{ username: string; password: string } | null>(null);
  const [busy, setBusy] = useState(false);
  const [message, setMessage] = useState("");
  const [pane, setPane] = useState<WorkspaceSettingsPane>("users");
  ...
  async function createUser(event: FormEvent) {
    ...
    try {
      await request("/api/v1/workspace/members", { method: "POST", body: JSON.stringify({ display_name: displayName, username, password, role }) });
      setLastAccess({ username, password }); setDisplayName(""); setUsername(""); setPassword(""); setRole("OPERATOR"); setMessage("Workspace user created and access assigned"); await loadMembers();
    } catch (error) { setMessage(error instanceof Error ? error.message : "Workspace user could not be created"); } finally { setBusy(false); }
  }
  async function updateMember(member: WorkspaceMember, nextRole: string) {
    setBusy(true); setMessage("");
    try { await request(`/api/v1/workspace/members/${member.operator_id}`, { method: "PUT", body: JSON.stringify({ role: nextRole }) }); setMessage("Workspace role updated"); await loadMembers(); } catch (error) { setMessage(error instanceof Error ? error.message : "Role could not be updated"); } finally { setBusy(false); }
  }
  async function removeMember(member: WorkspaceMember) {
    if (member.role === "OWNER" || !window.confirm(`Remove ${member.display_name || member.username} from this workspace?`)) return;
    setBusy(true); setMessage("");
    try { await request(`/api/v1/workspace/members/${member.operator_id}`, { method: "DELETE" }); setMessage("Workspace access removed"); await loadMembers(); } catch (error) { setMessage(error instanceof Error ? error.message : "Workspace access could not be removed"); } finally { setBusy(false); }
  }
  ...
  return <WorkspaceSettingsFrame identity={identity} adminMode workspaceName={selectedWorkspace?.name || "Workspace"} pane={pane} onPaneChange={setPane}>{pane === "profile" && <div className="settings-form-stack"><div className="settings-callout"><div className="callout-icon"><Building2 size={16} /></div><div><strong>Product-admin workspace profile</strong><p>Manage the selected workspace identity and lifecycle. Room TFNs and concurrency stay inside Room Settings.</p></div><span className="status live">Platform scope</span></div><div className="settings-form-card"><div className="subsection-head"><div><h3>Workspace identity</h3><p className="subtle">Changes apply only to the selected customer workspace.</p></div><ShieldCheck size={17} /></div>{selectedWorkspace ? <form className="form-grid workspace-profile-form" onSubmit={(event) => void saveWorkspace(event)}><label className="field-label">Workspace name<input className="field-control" value={policyName} onChange={(event) => setPolicyName(event.target.value)} /></label><label className="field-label">Status<select className="field-control" value={policyStatus} onChange={(event) => setPolicyStatus(event.target.value)}><option value="ACTIVE">Active</option><option value="SUSPENDED">Suspended</option></select></label><div className="editor-actions"><button className="button primary" disabled={busy}><Save size={14} /> Save profile</button></div></form> : <div className="session-empty"><Building2 size={17} /><span><strong>No workspace selected</strong><small>Use the workspace switcher to select one.</small></span></div>}</div>{message && <div className="settings-inline-message">{message}</div>}</div>}{pane === "roles" && <WorkspaceRolesPanel />}{pane === "users" && <div className="settings-form-stack"><div className="settings-callout"><div className="callout-icon"><Users size={16} /></div><div><strong>Customer access for this workspace</strong><p>Platform administrator identities are managed separately and are never mixed into this customer user list.</p></div><span className="status live">{members.length} member{members.length === 1 ? "" : "s"}</span></div><div className="settings-form-card"><div className="subsection-head"><div><h3>Invite a local workspace member</h3><p className="subtle">Create a local account with a temporary password and assign a workspace role.</p></div><UserPlus size={17} /></div><form className="form-grid workspace-member-form" onSubmit={(event) => void createUser(event)}><label className="field-label">Display name<input className="field-control" value={displayName} onChange={(event) => setDisplayName(event.target.value)} placeholder="Alex Morgan" required /></label><label className="field-label">Username<input className="field-control" value={username} onChange={(event) => setUsername(event.target.value)} placeholder="alex.morgan" required /></label><label className="field-label">Role<select className="field-control" value={role} onChange={(event) => setRole(event.target.value)}><option value="ADMIN">Workspace admin</option><option value="OPERATOR">Workspace operator</option><option value="VIEWER">Read-only viewer</option></select></label><label className="field-label">Temporary password<input className="field-control" type="password" minLength={8} value={password} onChange={(event) => setPassword(event.target.value)} placeholder="At least 8 characters" required /></label><div className="editor-actions"><button className="button primary" disabled={busy || !selectedWorkspaceID}>{busy ? "Adding…" : <><Plus size={14} /> Add member</>}</button></div></form></div>{lastAccess && <div className="settings-callout settings-credential-callout"><div className="callout-icon"><Check size={16} /></div><div><strong>Share these one-time access details</strong><p>{lastAccess.username} · temporary password ready to share securely.</p></div><button type="button" className="button ghost compact-button" onClick={() => void copyAccess()}>Copy details</button></div>}{message && <div className="settings-inline-message">{message}</div>}<div className="member-list">{members.map((member) => <div className="member-card" key={member.operator_id}><div className="member-avatar">{initials(member.display_name || member.username)}</div><div className="member-copy"><strong>{member.display_name || member.username}</strong><small>{member.username}</small></div><span className="role-chip">{workspaceRoleDescriptions[member.role]?.label || member.role}</span><span className={`member-status ${member.status === "ACTIVE" ? "active" : "paused"}`}>{member.status === "ACTIVE" ? "Active" : member.status}</span><div className="member-card-actions">{member.role === "OWNER" ? <span className="ownership-label">Owner</span> : <><select className="field-control compact-select" value={member.role} onChange={(event) => void updateMember(member, event.target.value)} disabled={busy} aria-label={`Role for ${member.display_name || member.username}`}><option value="ADMIN">Admin</option><option value="OPERATOR">Operator</option><option value="VIEWER">Viewer</option></select><button type="button" className="icon-button danger" onClick={() => void removeMember(member)} disabled={busy} title={`Remove ${member.display_name || member.username}`}><Trash2 size={14} /></button></>}</div></div>)}{!members.length && <div className="empty compact-empty">No workspace users yet.</div>}</div></div>}</WorkspaceSettingsFrame>;
  return (
