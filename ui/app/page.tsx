...
type Bridge = { id: string; name: string; status: string; created_at?: string };
type Participant = {
...
type RoomState = { workspace_id?: string; bridge_id: string; participants: Participant[]; speaker_requests?: SpeakerRequest[] };
type Contact = { name: string; phone_number: string; valid: boolean; reason?: string };
...
export default function Dashboard() {
  ...
  return <div className={`app-shell ${view === "room" ? "room-mode" : ""}`}>
    {view === "room" && selectedRoom && <RoomSidebar room={selectedRoom} tab={roomTab} connectedCount={selectedState?.participants.filter(isConnected).length || 0} liveCount={selectedState?.participants.filter(isLive).length || 0} onTab={setRoomTab} onBack={() => navigate("rooms")} />}
...
function RoomSidebar({ room, tab, connectedCount, liveCount, onTab, onBack }: { room: Bridge; tab: RoomTab; connectedCount: number; liveCount: number; onTab: (tab: RoomTab) => void; onBack: () => void }) {
  return <aside className="room-sidebar" aria-label="Conference room navigation">
    <button type="button" className="room-sidebar-back" onClick={onBack}><ArrowLeft size={14} /> <span>Back to workspace</span></button>
    ...
    <nav className="room-rail-nav" aria-label="Room sections">
      ...
      <RoomSidebarSection title="Outreach & media">
        <RoomTabButton active={tab === "dialer"} onClick={() => onTab("dialer")} icon={Phone} label="Dial participant" detail="Single call" />
    ...
    <div className="room-sidebar-footer"><StatusPill label={liveCount ? `${connectedCount} connected` : "Ready for a call"} state={liveCount ? "IN_BRIDGE" : undefined} /></div>
  </aside>;
}
...
function RoomPage({ room, state, tab, onTab, onRefresh, notify }: { room: Bridge; state: RoomState; tab: RoomTab; onTab: (tab: RoomTab) => void; onRefresh: () => void; notify: (message: string, tone?: "success" | "error") => void }) {
  ...
  return <div className="page-enter room-page"><div className="room-workspace"><section className="room-content"><PageHeader eyebrow={`Conference room · ${room.status || "ACTIVE"}`} title={room.name} description="Keep participant dialing and live controls isolated to this conference room." actions={<button type="button" className="button ghost" onClick={onRefresh}><RefreshCw size={15} /> Refresh</button>} /><section className="room-summary"><div><span className="eyebrow">Room overview</span><h2>{live.length ? "Live operations" : "Ready for a call"}</h2><p className="subtle">{live.length ? "Active participants appear here while they are connected or dialing." : "This room is empty until you start a participant call."}</p></div><div className="room-summary-stats"><span><strong>{connected.length}</strong><small>Connected</small></span><span><strong>{live.length - connected.length}</strong><small>In progress</small></span></div></section>{tab === "live" && <LiveControl live={live} requests={state.speaker_requests || []} onAction={performAction} onVolume={readVolume} onSpeakerAction={speakerAction} />}{tab === "dialer" && <DialerPage rooms={[room]} selectedRoomId={room.id} onRoomChange={() => undefined} onOpenRoom={() => { onTab("live"); onRefresh(); }} notify={notify} embedded />}{tab === "bulk" && <BulkPage rooms={[room]} selectedRoomId={room.id} onRoomChange={() => undefined} onOpenRoom={() => { onTab("live"); onRefresh(); }} notify={notify} embedded />}{tab === "activity" && <RoomActivity participants={state.participants} onAction={performAction} />}{tab === "settings" && <RoomSettingsPanel room={room} />}</section></div></div>;
}
...
function RoomActivity({ participants, onAction }: { participants: Participant[]; onAction: (participantId: string, action: "mute" | "unmute" | "drop" | "add") => Promise<void> }) {
  const [query, setQuery] = useState("");
  ...
  return <section className="panel room-activity-panel"><div className="panel-heading"><div><div className="eyebrow">Room history</div><h2>Participant activity</h2><p className="subtle">Review participants who have joined this room and add back a caller when needed.</p></div><label className="search-box"><Search size={15} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search participants" aria-label="Search room participant history" /></label></div>{filtered.length ? <div className="room-activity-table"><div className="room-activity-head"><span>Participant</span><span>Phone number</span><span>State</span><span>Action</span></div>{filtered.map((participant) => <div className="room-activity-row" key={`${participant.participant_id}-${participant.call_id}`}><span className="participant-identity"><span className="participant-avatar">{initials(participant.name || participant.phone_number)}</span><span><strong>{participant.name || "Unnamed participant"}</strong><small>{participant.phone_number}</small></span></span><span className="mono">{participant.phone_number}</span><StatusPill state={participant.state} /><span>{terminalStates.has(participant.state) ? <button type="button" className="text-button" onClick={() => void onAction(participant.participant_id, "add")}>Add back</button> : isLive(participant) ? <button type="button" className="text-button danger-text" onClick={() => void onAction(participant.participant_id, "drop")}>Drop</button> : <span className="muted-action">No action</span>}</span></div>)}</div> : <EmptyState icon={Activity} title="No participant activity" description={participants.length ? "No participants match the current search." : "This room has no participant history yet."} />}</section>;
}

function RoomSettingsPanel({ room }: { room: Bridge }) {
  return <section className="room-settings-grid"><div className="panel settings-main"><div className="settings-section-title"><div><div className="eyebrow">Room preferences</div><h2>{room.name}</h2><p className="subtle">A focused customer view of this conference room and its participant experience.</p></div><ShieldCheck size={18} /></div><div className="settings-fields"><div><span>Room status</span><strong>{room.status || "ACTIVE"}</strong></div><div><span>Room access</span><strong>Workspace members</strong></div><div><span>Participant controls</span><strong>Mute · volume · remove</strong></div><div><span>Media experience</span><strong>Phone audio</strong></div></div></div><div className="panel settings-main"><div className="settings-section-title"><div><div className="eyebrow">Conference experience</div><h2>Built for calm live calls</h2><p className="subtle">Participants stay isolated to this room, while operators can manage the conversation without leaving the room workspace.</p></div><Users size={18} /></div><div className="policy-list"><PolicyRow icon={Users} title="Room participants" value="Room only" detail="Only callers placed into this room appear here" /><PolicyRow icon={Mic} title="Operator controls" value="Available live" detail="Mute, unmute, volume, and remove callers" /><PolicyRow icon={ShieldCheck} title="Workspace access" value="Protected" detail="Only authorized workspace members can operate rooms" /></div></div></section>;
}

function LiveControl({ live, requests, onAction, onVolume, onSpeakerAction }: { live: Participant[]; requests: SpeakerRequest[]; onAction: (participantId: string, action: "mute" | "unmute" | "drop" | "add") => Promise<void>; onVolume: (participantId: string) => Promise<void>; onSpeakerAction: (requestID: string, action: "grant" | "withdraw") => Promise<void> }) {
...
function DialerPage({ rooms, selectedRoomId, onRoomChange, onOpenRoom, notify, embedded = false }: { rooms: Bridge[]; selectedRoomId: string; onRoomChange: (value: string) => void; onOpenRoom: (id: string) => void; notify: (message: string, tone?: "success" | "error") => void; embedded?: boolean }) {
  ...
  async function submit(event?: FormEvent) {
    ...
    if (!selectedRoomId || !phone.trim()) { notify("Choose a room and enter a phone number", "error"); return; }
    setBusy(true);
    try {
      await request(`/api/v1/bridges/${selectedRoomId}/dial`, { method: "POST", body: JSON.stringify({ name: name.trim() || phone.trim(), phone_number: phone.trim() }) });
...
function BulkPage({ rooms, selectedRoomId, onRoomChange, onOpenRoom, notify, embedded = false }: { rooms: Bridge[]; selectedRoomId: string; onRoomChange: (value: string) => void; onOpenRoom: (id: string) => void; notify: (message: string, tone?: "success" | "error") => void; embedded?: boolean }) {
  ...
  async function parseFile(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const parsed = rows.map((row) => {
        ...
        return { name, phone_number: phone, valid, reason: !phone ? "Missing phone number" : !valid ? "Invalid phone number" : undefined };
      });
      setContacts(parsed);
  ...
  async function launch() {
    ...
    setBusy(true);
    try {
      const result = await request<{ batch_id: string }>(`/api/v1/bridges/${selectedRoomId}/batches`, { method: "POST", body: JSON.stringify({ contacts: validContacts.map(({ name, phone_number }) => ({ name, phone_number })) }) });
      notify(`Batch ${result.batch_id.slice(0, 8)} queued with controlled concurrency`);
      onOpenRoom(selectedRoomId);
    } catch (error) { notify(error instanceof Error ? error.message : "Batch could not be queued", "error"); } finally { setBusy(false); }
  }
  return <div className={`page-enter task-page ${embedded ? "embedded-task" : ""}`}><PageHeader eyebrow={embedded ? "Room operation · Contact list" : "Tools · Bulk outreach"} title="Upload a contact list" description="Preview XLSX or CSV contacts locally, validate phone numbers, and queue the valid rows into a conference room." actions={!embedded && <button type="button" className="button ghost" onClick={() => onOpenRoom(selectedRoomId)} disabled={!selectedRoomId}><Building2 size={15} /> Open room</button>} /><div className="panel bulk-card"><div className="bulk-toolbar"><div className="task-card-heading"><span className="task-icon"><FileSpreadsheet size={17} /></span><div><div className="eyebrow">Controlled outreach</div><h2>Prepare participants</h2></div></div><label className="button primary upload-button"><Upload size={15} /> Choose XLSX / CSV<input type="file" accept=".xlsx,.xls,.csv" onChange={(event) => void parseFile(event)} /></label></div><div className="bulk-options"><label className="field-label">Target room<select className="field-control" value={selectedRoomId} onChange={(event) => onRoomChange(event.target.value)} required><option value="">Choose a room</option>{rooms.map((room) => <option key={room.id} value={room.id}>{room.name}</option>)}</select></label><div className="bulk-protection"><ShieldCheck size={16} /><span><strong>Local preview first</strong><small>Rows stay in the browser until you queue the batch.</small></span></div></div>{contacts.length ? <><div className="preview-header"><div><div className="eyebrow">Import preview</div><h3>{validContacts.length} ready of {contacts.length} rows</h3></div><span className="preview-summary"><Check size={13} /> Validated locally</span></div><div className="contact-table"><div className="contact-table-head"><span>Name</span><span>Phone number</span><span>Validation</span></div>{contacts.slice(0, 100).map((contact, index) => <div className="contact-row" key={`${contact.phone_number}-${index}`}><span>{contact.name || "Unnamed contact"}</span><span className="mono">{contact.phone_number || "—"}</span><span className={contact.valid ? "valid" : "invalid"}>{contact.valid ? <><Check size={13} /> Ready</> : <><AlertCircle size={13} /> {contact.reason}</>}</span></div>)}</div><div className="bulk-footer"><span>{contacts.length > 100 ? "Showing first 100 rows · " : ""}{validContacts.length} valid contacts will be queued.</span><button type="button" className="button primary" onClick={() => void launch()} disabled={busy || !selectedRoomId || !validContacts.length}><Phone size={15} /> {busy ? "Queueing…" : "Queue outbound calls"}</button></div></> : <EmptyState icon={FileSpreadsheet} title="No contact list loaded" description="Choose an XLSX or CSV file with Name and Phone Number columns to preview the batch here." action={<div className="file-format-note"><span>Name</span><span>Phone Number</span><span>CSV / XLSX</span></div>} />}</div></div>;
}
