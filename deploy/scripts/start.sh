...
DATA_ROOT="/data"
RUNTIME="$DATA_ROOT/state/runtime"
...
require_file() { [[ -x "$1" ]] || { echo "missing executable: $1; run /data/deploy/scripts/build.sh" >&2; exit 1; }; }
...
require_file "$COMPONENTS_ROOT/phonarch-sip-gateway/bin/sipgo-lb-$BIN_SUFFIX"
require_file "$COMPONENTS_ROOT/phonarch-pbx-sidecar/bin/pbx-sidecar-$BIN_SUFFIX"
require_file "$COMPONENTS_ROOT/phonarch-platform-api/bin/control-api-$BIN_SUFFIX"
...
for index in "${!SIPGO_BINDS[@]}"; do
  ...
  start_bg "$label" env REDIS_ADDR="$REDIS_ADDR" SIPGO_NODE_ID="$edge_id" SIPGO_ADVERTISE_IP="$advertise_ip" SIPGO_PUBLIC_IP="$public_ip" SIPGO_LISTEN="$bind" SIPGO_TCP_LISTEN="$tcp_bind" SIPGO_HTTP="$http_bind" SIPGO_ADVERTISE_HTTP="$(http_url "$http_bind")" "$COMPONENTS_ROOT/phonarch-sip-gateway/bin/sipgo-lb-$BIN_SUFFIX"
done

export CONTROL_API_LISTEN="${CONTROL_API_LISTEN:-127.0.0.1:8081}"
[[ -n "${DATABASE_URL:-}" ]] || { echo "DATABASE_URL must be configured" >&2; exit 1; }
start_bg control-api "$COMPONENTS_ROOT/phonarch-platform-api/bin/control-api-$BIN_SUFFIX"
...
elif [[ "${PHONARCH_USE_MOCK_RUSTPBX:-0}" == "1" && -x "$COMPONENTS_ROOT/phonarch-rustpbx-lab/bin/mock-rustpbx-$BIN_SUFFIX" ]]; then
  for index in "${!CORE_PRIVATE_IPS[@]}"; do
    private_ip=${CORE_PRIVATE_IPS[$index]}
    start_bg "rustpbx-$(sanitize_id "$private_ip")" env MOCK_LISTEN="${CORE_RUSTPBX_CONTROLS[$index]}" MOCK_SIP_LISTEN="${private_ip}:${CORE_SIP_PORTS[$index]}" "$COMPONENTS_ROOT/phonarch-rustpbx-lab/bin/mock-rustpbx-$BIN_SUFFIX"
  done
else
